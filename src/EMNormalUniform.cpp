...
// title E-Step for Expectation Maximization - Normal+Uniform Distribution
...
Rcpp::List estepNU(const Rcpp::List parmlist, const arma::vec xi, std::string type,
...
// M-Step with Numerical Optimization for Expectation Maximization - Normal Distribution
//
...
Rcpp::List mstepNU(Rcpp::List eout){
...
// [[Rcpp::export]]
Rcpp::List emstepNU(Rcpp::List parmlist, const arma::vec xi, int niter, double epsilon, arma::vec trunc,  std::string type = "free", bool glreturn = false){
   // Set up progress

   Rcpp::List mint = parmlist;
   // E-step
   Rcpp::List eint = estepNU(mint, xi, type, trunc);
   double lnlikeint = llcalcfinalNU(eint);
   ...
   for(int j = 0; j < (niter + 1); j++){
     ...
     // M-step
     Rcpp::List moutip = mstepNU(eint);


     // E-step
     Rcpp::List eoutip = estepNU(moutip, xi, type, trunc);
     double lnlikeip = llcalcfinalNU(eoutip);
