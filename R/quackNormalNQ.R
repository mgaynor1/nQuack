  ...
  # Input parameters
    ...
    # Set up parm list for BIC calculations
      ...
      if(free == TRUE){
        ...
        LL1 <- data.frame(LL = (emstepN(p, xi, niter, epsilon, trunc))$loglikelihood, type = "free", mixture = "free", distribution = "normalNQ")
        LL2 <- data.frame(LL = (emstepNU(pu, xi, niter, epsilon, trunc))$loglikelihood, type = "free", mixture = "free", distribution = "normalNQ-uniform")
      ...
      d <- foreach(type=1:3, .combine='rbind', .options.nws=opts) %:%
            foreach::foreach(iter = 1:5, .combine='rbind', .packages = "nQuack") %dopar% {
              ...
              obj <-  nQuack::emstepN(set[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
              BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistN[[type]][iter]))
              data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normalNQ", BIC = BICout)
            }
      message("Calculating likelihood of each mixture with a normal + uniform distibution.")
        ...
        du <- foreach(type=1:3, .combine='rbind', .options.nws=opts) %:%
          foreach::foreach(iter = 1:5, .combine='rbind', .packages = "nQuack") %dopar% {
            obj <-  nQuack::emstepNU(setU[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
            BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistU[[type]][iter]))
            data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normalNQ-uniform", BIC = BICout)
          }
        ...
        for(type in 1:3){
          for(iter in 1:5){
              obj <-  nQuack::emstepN(set[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
              BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistN[[type]][iter]))
             d <- rbind(d, data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normalNQ", BIC = BICout))
        }
        }
          ...
          for(type in 1:3){
            for(iter in 1:5){
              obj <-  nQuack::emstepNU(setU[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
              BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistU[[type]][iter]))
              du <- rbind(du, data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normalNQ-uniform", BIC = BICout))
            }
          }
        }
