...
quackNormal <- function(xm, samplename, cores, parallel= FALSE,
  ...
  setU <- list()
   ...
   if(lowvar == FALSE){
      ...
      ## Model Params
      pu =  list(avec = c(0.11, 0.22, 0.34, 0.22, 0.11, 0.0), mvec = c(0.20, 0.33, 0.50, 0.67, 0.80), svec = c(0.01, 0.01, 0.01, 0.01, 0.01));
  ...
  LL1 <- data.frame(LL = (emstepNUA(pu, xi, niter, epsilon, trunc))$loglikelihood, type = "free", mixture = "free", distribution = "normal-uniform")
  LL2 <- data.frame(LL = (emstepNA(p, xi, niter, epsilon, trunc))$loglikelihood, type = "free", mixture = "free", distribution = "normal")
  ...
  message("Calculating likelihood of each mixture with a normal distibution.")
  d <- foreach(type=1:3, .combine='rbind', .options.nws=opts) %:%
      foreach::foreach(iter = 1:5, .combine='rbind', .packages = "nQuack") %dopar% {
        obj <-  nQuack::emstepNA(set[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
        BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistN[[type]][iter]))
        data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normal", BIC = BICout)
      }

   opts <- list(chunkSize=2)
   message("Calculating likelihood of each mixture with a normal+uniform distibution.")

  du <- foreach(type=1:3, .combine='rbind', .options.nws=opts) %:%
    foreach::foreach(iter = 1:5, .combine='rbind', .packages = "nQuack") %dopar% {
      obj <-  nQuack::emstepNUA(setU[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
      BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistU[[type]][iter]))
      data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normal-uniform", BIC = BICout)
    }
  ...
  parallel::stopCluster(cl)
    ...
    for(type in 1:3){
      for(iter in 1:5){
          obj <-  nQuack::emstepNA(set[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
          BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistN[[type]][iter]))
           d <-  rbind(d,  data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normal", BIC = BICout))
    ...
    for(type in 1:3){
      for(iter in 1:5){
        obj <-  nQuack::emstepNUA(setU[[iter]], xi, niter, epsilon, trunc,  type = bvec[type])
        BICout <- ((-2*obj$loglikelihood) + (lnn*parmlistU[[type]][iter]))
       du <- rbind(du, data.frame(LL=obj$loglikelihood, type=bvec[type], mixture=mixturekey[iter], distribution = "normal-uniform", BIC = BICout))
      }
    }
